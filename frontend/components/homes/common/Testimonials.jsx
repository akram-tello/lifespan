...
export default function Testimonials() {
  ...
  useEffect(() => {
    async function fetchTestimonials() {
      try {
        const response = await fetch('https://mindvally.akramtello.com/wp-json/wp/v2/testimonials?_fields=acf,&acf_format=standard');
        const data = await response.json();
        setTestimonials(data.map((item) => item.acf));
      } catch (error) {
    ...
    <div className="testimonial-area-1 space bg-theme shape-mockup-wrap">
      ...
      >
        <Image
          width={838}
          height={716}
          src="/assets/img/testimonial/testi.jpeg"
          alt="img"
          loading="lazy" 
        />
      </div>
          ...
          <div className="col-lg-6">
            ...
            <Slider className="row global-carousel testi-slider1" {...settings}>
              {testimonials.map((testimonial, index) => (
                <div key={index} className="col-lg-4 sliderItem">
